...
public class ProductImpl implements ProductDao{
	...
	private static long foodKey=0;
	private static long apparelKey=0;
	private static long electronicsKey=0;
	private Hashtable<Long, FoodItems> foodht = new Hashtable<>();
	private Hashtable<Long, Apparel> apparelht = new Hashtable<>();
	private Hashtable<Long, Electronics> electronicsht = new Hashtable<>();



	@Override
	public boolean addProduct(List<Product> products) {
		// store in Hashtable; stores data in heap; one list divided into three HTs
		...
		for(Product product: products) {
			...
			if(product instanceof Apparel) {
				apparelKey++;
				apparelht.put(foodKey, (Apparel) product);
			}
			if(product instanceof Electronics) {
				electronicsKey++;
				electronicsht.put(foodKey, (Electronics) product);
			}
		}
		return true;
	}
	...
	public List<Product> getAllProducts(int type) {
		...
		if(type==1) {
			Collection<FoodItems> values = foodht.values();  //Need to convert Hashtable (map type) to Arraylist (list type)
		    List<Product> prodList = new ArrayList<>(values);
			return prodList ;
		}
		else if (type==2) {
			Collection<Apparel> values = apparelht.values();
		    List<Product> prodList = new ArrayList<>(values);
			return prodList ;
		}
		else if (type==3) {
			Collection<Electronics> values = electronicsht.values();
		    List<Product> prodList = new ArrayList<>(values);
			return prodList ;
		}
		else {
